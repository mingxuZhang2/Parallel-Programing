...
void backwardPropagationMPI(const vector<vector<double>>& inputs, const vector<vector<double>>& targets,
    vector<vector<double>>& output_weights, vector<vector<double>>& hidden_weights,
    vector<double>& output_bias, vector<double>& hidden_bias, int rank, int worldSize) {
    ...
    // 平均分配训练样本给各个进程
    int chunkSize = numSamples / worldSize;
    int start = rank * chunkSize;
    int end = (rank == worldSize - 1) ? numSamples : (rank + 1) * chunkSize;
    ...
    // 进行结果的全局归约
    MPI_Allreduce(MPI_IN_PLACE, output_weights.data(), outputSize * hiddenSize, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, hidden_weights.data(), hiddenSize * inputSize, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, output_bias.data(), outputSize, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, hidden_bias.data(), hiddenSize, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
}
int main(int argc, char** argv) {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        backwardPropagationMPI(inputs, targets, outputWeights, hiddenWeights, outputBias, hiddenBias, rank, worldSize);
    }
